...
//	    String sqlSelect = "SELECT us_nome FROM tb_usuario WHERE us_nome like ? ";
		...
		String sqlSelect = "SELECT u.us_nome, u.us_senha, u.us_nivel, c.cd_id, c.cd_nome, ifnull((SELECT ac_libera FROM TB_ACESSO WHERE cd_id = c.cd_id AND us_nome = u.us_nome), 'B') AS ac_libera"+
	    ...
	    try{
			...
			while(rs.next()) {	
				if(!rs.getString("us_nome").equals(usuarioAtual)){
					...
					user.setSenha(rs.getString("us_senha"));
					user.setUs_nivel(rs.getString("us_nivel"));
...
//			return userTO;
	...
	//INSERE NOVO USUARIO
	public UsuarioTO insere(String us_nome, String us_senha, String us_nivel) throws Exception{
		Connection conn = null;
	    String sqlInsert = "INSERT INTO TB_USUARIO(us_nome, us_senha, us_nivel) VALUES(?, ?, ?) ";
	    PreparedStatement stm = null;
	...
	public UsuarioTO altera(String us_nome, String us_nome_old, String us_senha, String us_nivel) throws Exception{
		Connection conn = null;
	    String sqlUpdate = "UPDATE TB_USUARIO SET us_nome = ?, us_senha = ?, us_nivel = ? WHERE us_nome = ?";
	    PreparedStatement stm = null;
